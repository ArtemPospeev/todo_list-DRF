...
class Project(CustomBaseModel):
    project_number = models.PositiveIntegerField(unique=True, editable=False, verbose_name='Project number', default=10)
    ...
    def save(self, *args, **kwargs):
        if self._state.adding:
            last_number = Project.objects.all().aggregate(largest=models.Max('project_number'))['largest']
...
class ToDo(CustomBaseModel):
    ...
    task_number = models.PositiveIntegerField(unique=True, editable=False, verbose_name='Task number', default=1000)
    ...
    def save(self, *args, **kwargs):
        if self._state.adding:
            last_number = ToDo.objects.all().aggregate(largest=models.Max('task_number'))['largest']
            if last_number is not None:
